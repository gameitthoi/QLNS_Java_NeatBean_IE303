...
//        nếu sách cần lấy có giá trị là 0 thì không cho lấy
            ...
            for (int i=0;i<dtmInSanPham.getRowCount();i++) {
                if (dtmInSanPham.getValueAt(i, 0).equals(SanphamTable.getValueAt(SanphamTable.getSelectedRow(), 0))) vitriSach=i;
            }
            //nếu đã có trong hóa đơn thì cập nhật lại số lượng mã vạch
            if (vitriSach!=-1){
                dtmInSanPham.setValueAt(Integer.parseInt(SLSPInput.getValue().toString())+Integer.parseInt(dtmInSanPham.getValueAt(vitriSach, 3).toString()), vitriSach, 3);
            }
            //nếu chưa có thì tạo dòng mới
            else{                        
